...
Switch::Switch(const std::string &path, const std::string &nodeNamespace, const std::string &type, const std::atomic_bool *frontendConnected) : Flows::INode(path, nodeNamespace, type, frontendConnected) {
}
...
bool Switch::init(const Flows::PNodeInfo &info) {
  try {
...
bool Switch::isTrue(Flows::PVariable &value) {
  if (value->type != Flows::VariableType::tBoolean) {
    switch (value->type) {
      case Flows::VariableType::tArray: return !value->arrayValue->empty();
      case Flows::VariableType::tBase64: return !value->stringValue.empty();
      case Flows::VariableType::tBinary: return !value->binaryValue.empty();
      case Flows::VariableType::tBoolean: break;
      case Flows::VariableType::tFloat: return (bool)value->floatValue;
      case Flows::VariableType::tInteger: return (bool)value->integerValue;
      case Flows::VariableType::tInteger64: return (bool)value->integerValue64;
      case Flows::VariableType::tString: return !value->stringValue.empty();
      case Flows::VariableType::tStruct: return !value->structValue->empty();
      case Flows::VariableType::tVariant: break;
      case Flows::VariableType::tVoid: return false;
    }
  }
...
bool Switch::match(const Flows::PNodeInfo &nodeInfo, Rule &rule, Flows::PVariable &value) {
  try {
    ...
    switch (rule.t) {
      case RuleType::tEq: if (*value == *rule.v) return true;
        return false;
      case RuleType::tNeq: if (*value == *rule.v) return false;
        return true;
      case RuleType::tLt: if (value->type == rule.vt && *value < *rule.v) return true;
        return false;
      case RuleType::tLte: if (value->type == rule.vt && *value <= *rule.v) return true;
        return false;
      case RuleType::tGt: if (value->type == rule.vt && *value > *rule.v) return true;
        return false;
      case RuleType::tGte: if (value->type == rule.vt && *value >= *rule.v) return true;
        return false;
      case RuleType::tBtwn:
        ...
        return false;
      case RuleType::tCont: return value->stringValue.find(rule.v->toString()) != std::string::npos;
      case RuleType::tRegex: if (value->type != Flows::VariableType::tString) value->stringValue = value->toString();
        return std::regex_match(value->stringValue, rule.regex);
      case RuleType::tTrue: return isTrue(value);
      case RuleType::tFalse: return !isTrue(value);
      case RuleType::tNull: return value->type == Flows::VariableType::tVoid;
      case RuleType::tNnull: return value->type != Flows::VariableType::tVoid;
      case RuleType::tElse: return true;
    }
  }
  catch (const std::exception &ex) {
    _out->printEx(__FILE__, __LINE__, __PRETTY_FUNCTION__, ex.what());
        ...
        if (_outputTrue) {
          ...
          if (!_changesOnly || *(_rules.at(i).previousOutput) != *(currentMessage)) {
            ...
            if (_staticOnly) {
              ...
              outputMessage->structValue->emplace("payload", _value);
