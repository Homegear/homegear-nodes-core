...
bool MyNode::init(Flows::PNodeInfo info)
{
	try
	{
		...
		_lastGreaterOrEqual = isGreaterOrEqual(_input1, _input2);
...
bool MyNode::isGreaterOrEqual(Flows::PVariable& input1, Flows::PVariable& input2)
{
	try
	{
		...
		if(input1->type == Flows::VariableType::tInteger || input1->type == Flows::VariableType::tInteger64)
		{
			...
			{
				greaterOrEqual = input1->integerValue64 >= input2->integerValue64;
			}
			else if(input2->type == Flows::VariableType::tFloat || input2->type == Flows::VariableType::tFloat)
			{
				greaterOrEqual = input1->integerValue64 >= input2->floatValue;
			}
		}
		else if(input1->type == Flows::VariableType::tFloat || input1->type == Flows::VariableType::tFloat)
		{
			...
			{
				greaterOrEqual = input1->floatValue >= input2->floatValue;
			}
			else if(input2->type == Flows::VariableType::tInteger || input2->type == Flows::VariableType::tInteger64)
			{
				greaterOrEqual = input1->floatValue >= input2->integerValue64;
			}
		}
...
void MyNode::input(Flows::PNodeInfo info, uint32_t index, Flows::PVariable message)
{
	try
	{
		...
		bool greaterOrEqual = isGreaterOrEqual(_input1, _input2);
		bool doOutput = !_outputChangesOnly;
		if(!greaterOrEqual && !_outputFalse) doOutput = false;
		else if(greaterOrEqual != _lastGreaterOrEqual) doOutput = true;
		_lastGreaterOrEqual = greaterOrEqual;
		if(doOutput)
		{
			Flows::PVariable outputMessage = std::make_shared<Flows::Variable>(Flows::VariableType::tStruct);
			outputMessage->structValue->emplace("payload", std::make_shared<Flows::Variable>(greaterOrEqual));
