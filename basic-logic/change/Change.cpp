...
bool Change::init(const Flows::PNodeInfo &info) {
  try {
      ...
      if (rule.t == RuleType::tDelete) {
        ...
        if (rule.fromRegexSet) {
          ...
          if (fromValue->type == Flows::VariableType::tString) {
            ...
            if ((haystack->type == Flows::VariableType::tBoolean && haystack->booleanValue == fromValue->booleanValue) ||
                (haystack->type == Flows::VariableType::tInteger64 && haystack->integerValue64 == fromValue->integerValue64) ||
                (haystack->type == Flows::VariableType::tFloat && haystack->floatValue == fromValue->floatValue)) haystack = toValue;
          }
        }
