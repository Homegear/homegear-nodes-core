...
bool MyNode::isLessThan(Flows::PVariable& input1, Flows::PVariable& input2)
{
	try
	{
		bool lessThan = false;
		...
		if(input1->type == Flows::VariableType::tInteger || input1->type == Flows::VariableType::tInteger64)
		{
			...
			{
				lessThan = input1->integerValue64 < input2->integerValue64;
			}
			else if(input2->type == Flows::VariableType::tFloat || input2->type == Flows::VariableType::tFloat)
			{
				lessThan = input1->integerValue64 < input2->floatValue;
			}
		}
		else if(input1->type == Flows::VariableType::tFloat || input1->type == Flows::VariableType::tFloat)
		{
			...
			{
				lessThan = input1->floatValue < input2->floatValue;
			}
			else if(input2->type == Flows::VariableType::tInteger || input2->type == Flows::VariableType::tInteger64)
			{
				lessThan = input1->floatValue < input2->integerValue64;
			}
		}
		return lessThan;
	}
...
void MyNode::input(Flows::PNodeInfo info, uint32_t index, Flows::PVariable message)
{
	try
	{
		...
		if(!lessThan && !_outputFalse) doOutput = false;
		else if(lessThan != _lastLessThan) doOutput = true;
		_lastLessThan = lessThan;
		if(doOutput)
		{
			Flows::PVariable outputMessage = std::make_shared<Flows::Variable>(Flows::VariableType::tStruct);
			outputMessage->structValue->emplace("payload", std::make_shared<Flows::Variable>(lessThan));
