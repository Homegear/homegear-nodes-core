...
class BlockValueChange(unittest.TestCase):
    ...
    def test_negativeNumbersSameValue(self):
        ...
        inputHistory = hg.getNodeVariable(n2, "inputHistory0")
        self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
        self.assertEqual(round(inputHistory[0][1]['payload'], 7), (values[0] * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(values[0] * -1)}")
    ...
    def test_negativeNumbersBlockNone(self):
        ...
        inputHistory = hg.getNodeVariable(n2, "inputHistory0")
        self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
        self.assertEqual(round(inputHistory[1][1]['payload'], 7), (values[0] * -1), f"Payload is {inputHistory[1][1]['payload']}, but should be {(values[0] * -1)}")
        self.assertEqual(round(inputHistory[0][1]['payload'], 7), (values[1] * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(values[1] * -1)}")
    ...
    def test_negativeNumbersBlockNoneExtended(self):
        for value in values:
            ...
            inputHistory = hg.getNodeVariable(n2, "inputHistory0")
            self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
            self.assertEqual(round(inputHistory[0][1]['payload'], 7), (value * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(value * -1)}")
    ...
    def test_mixedNumbers(self):
        for i in range(len(values)):
            if i % 2:
                ...
                inputHistory = hg.getNodeVariable(n2, "inputHistory0")
                self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
                self.assertEqual(round(inputHistory[0][1]['payload'], 7), (values[i] * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(values[i] * -1)}")
            else:
...
class BlockValueChangeMultipleInputs(unittest.TestCase):
    ...
    def test_negativeNumbersSameValue(self):
        for i in range(inputs):
            for j in range(10):
                hg.setNodeVariable(n1, ("fixedInput" + str(i)), {"payload": (values[0] * -1)})
            time.sleep(1)
            inputHistory = hg.getNodeVariable(n2, ("inputHistory" + str(i)))
            self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
            self.assertEqual(round(inputHistory[0][1]['payload'], 7), (values[0] * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(values[0] * -1)}")
    ...
    def test_negativeNumbersBlockNone(self):
        for i in range(inputs):
            hg.setNodeVariable(n1, ("fixedInput" + str(i)), {"payload": (values[0] * -1)})
            hg.setNodeVariable(n1, ("fixedInput" + str(i)), {"payload": (values[1] * -1)})
            time.sleep(1)
            inputHistory = hg.getNodeVariable(n2, ("inputHistory" + str(i)))
            self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
            self.assertEqual(round(inputHistory[1][1]['payload'], 7), (values[0] * -1), f"Payload is {inputHistory[1][1]['payload']}, but should be {(values[0] * -1)}")
    ...
    def test_negativeNumbersBlockNoneExtended(self):
        for i in range(inputs):
            for value in values:
                hg.setNodeVariable(n1, ("fixedInput" + str(i)), {"payload": (value * -1)})
                time.sleep(1)
                inputHistory = hg.getNodeVariable(n2, ("inputHistory" + str(i)))
                self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
                self.assertEqual(round(inputHistory[0][1]['payload'], 7), (value * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(value * -1)}")
    ...
    def test_mixedNumbers(self):
        for i in range(inputs):
            for j in range(len(values)):
                if j % 2:
                    hg.setNodeVariable(n1, ("fixedInput" + str(i)), {"payload": (values[j] * -1)})
                    time.sleep(1)
                    inputHistory = hg.getNodeVariable(n2, ("inputHistory" + str(i)))
                    self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
                    self.assertEqual(round(inputHistory[0][1]['payload'], 7), (values[j] * -1), f"Payload is {inputHistory[0][1]['payload']}, but should be {(values[j] * -1)}")
                else:
                    hg.setNodeVariable(n1, ("fixedInput" + str(i)), {"payload": values[j]})
                    time.sleep(1)
                    inputHistory = hg.getNodeVariable(n2, ("inputHistory" + str(i)))
                    self.assertTrue(len(inputHistory) >= 1, f"No message was passed on. Length is {len(inputHistory)}")
                    self.assertEqual(inputHistory[0][1]['payload'], values[j], f"Payload is {inputHistory[0][1]['payload']}, but should be {values[j]}")
